...
export default {
  ...
  computed: {
    ...
    canCreate() {
      let result = false;
      if (
          this.superOptions.currentParentRel?.field?.fieldExtras?.relationRules
              ?.creatable
      ) {
        result =
            this.superOptions.currentParentRel.field.fieldExtras.relationRules.creatable(
    ...
    canCreateMsgType() {
      ...
      if (this.superOptions.currentParentRel?.currentParentRecord) {
        if (
            this.superOptions.currentParentRel?.currentParentRecord.model.name ==
            "DBProviderGroup"
        ) {
          result = "provider";
        } else if (
    ...
    canCreatePart2() {
      ...
      if (
        ...
        result =
            this.canCreate || !!this.customerGroupsAssociatedWithUser.length;
      }
      return result;
    },
    canCreateMsg() {
